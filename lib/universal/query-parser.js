...
exports.UniversalQueryParser = void 0;
...
class UniversalQueryParser {
    ...
    async parse(naturalLanguageQuery) {
...
Be generous with variations and synonyms. Include confidence scores 0-1.`;
    ...
    async classifyIntent(query, entities) {
...
Return JSON:
{
  ...
  "reasoning": "User wants to understand business evolution over time"
    ...
    async determineScope(query, intent) {
...
Return JSON:
{
  ...
  "depth": "moderate"
    ...
    determineComplexity(query, intent, entities) {
        const indicators = {
    ...
    calculateConfidence(entities, intent, scope) {
        const scores = [
    ...
    fallbackIntentClassification(query, entities) {
        const lowerQuery = query.toLowerCase();
        let primary = 'business_overview';
    ...
    fallbackScopeAnalysis(query, intent) {
        return {
...
exports.UniversalQueryParser = UniversalQueryParser;
