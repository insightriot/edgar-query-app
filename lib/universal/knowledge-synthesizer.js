...
exports.KnowledgeSynthesizer = void 0;
...
class KnowledgeSynthesizer {
    ...
    async synthesizeAnswer(query, knowledge) {
    ...
    async generateNarrative(query, knowledge) {
...
Generate a comprehensive response:`;
    ...
    async structureSupportingData(query, knowledge) {
        const data = {};
    ...
    generateComparisonTables(companies) {
        ...
        const table = {
            ...
            rows: companies.map(company => [
                ...
                company.risk.risk_factors.slice(0, 2).map((r) => r.category).join(', ') || 'N/A'
            ]),
    ...
    generateFinancialMetricsTable(companies) {
        return {
    ...
    generateRiskAnalysisTable(companies) {
        const riskData = [];
        companies.forEach(company => {
            if (company.risk.risk_factors.length > 0) {
                company.risk.risk_factors.forEach((risk) => {
                    riskData.push([
                        company.identity.name,
                        risk.category,
                        risk.severity,
    ...
    assessAnswer(query, knowledge, narrative) {
        const assessment = {
    ...
    calculateAnswerConfidence(query, knowledge, narrative) {
        let confidence = knowledge.confidence;
        // Adjust based on query complexity
        if (query.complexity === 'research')
            confidence *= 0.8;
        else if (query.complexity === 'analytical')
            confidence *= 0.9;
        ...
        if (narrative.length < 100)
            confidence *= 0.6;
        if (narrative.includes('could not'))
            confidence *= 0.7;
        return Math.max(0.1, Math.min(1.0, confidence));
    }
    calculateAnswerCompleteness(query, knowledge) {
    ...
    identifyAssumptions(query, knowledge) {
        const assumptions = [];
    ...
    identifyBiasRisks(query, knowledge) {
        const biasRisks = [];
    ...
    generateFollowUpSuggestions(query, knowledge) {
        const suggestions = {
            suggested_queries: [],
            related_topics: [],
            deeper_analysis: [],
    ...
    generateFallbackNarrative(query, knowledge) {
        if (knowledge.companies.length === 0) {
    ...
    generateFilingLookupResponse(query, knowledge) {
        const company = knowledge.companies[0];
